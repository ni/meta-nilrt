...
do_install:append() {
   ...
   install -m 0755 ${UNPACKDIR}/bolt ${D}${sysconfdir}/init.d/bolt

   # Move boltctl to a directory that's not in PATH and install a wrapper
   # in its place
   mv ${D}${bindir}/boltctl ${D}${libexecdir}/
   install -m 0755 ${UNPACKDIR}/boltctl ${D}${bindir}/

   # Overwrite udev rules file so it works on systems without systemd
   install -m 0644 ${UNPACKDIR}/90-bolt.rules ${D}${base_libdir}/udev/rules.d/

   # Overwrite dbus config file to allow only root to run privileged commands
   install -m 0644 ${UNPACKDIR}/org.freedesktop.bolt.conf ${D}${datadir}/dbus-1/system.d/
